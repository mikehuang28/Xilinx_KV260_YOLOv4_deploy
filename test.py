# modified test.py
# original version:https://github.com/WongKinYiu/PyTorch_YOLOv4/blob/master/test.py
import argparse
import glob
import json
import os
...
def attempt_load(
    ...
    fuse=True,
    imgsz=640,
    force_reexport_deployable_model=False,
):
    ...
    for w in weights if isinstance(weights, list) else [weights]:
        ...
        if "qat_model_quant_info" in ckpt:
            if force_reexport_deployable_model:
                ...
                im = torch.zeros(1, 3, imgsz, imgsz).to(
                    next(_model_ckpt.parameters()).device
                ...
                _trainable_model = qat_processor.trainable_model(
                    allow_reused_module=False
                )
                _trainable_model.load_state_dict(
                    ckpt["qat_ema_state_dict"], strict=True
                )
                _deployable_net = qat_processor.convert_to_deployable(
                    _trainable_model, calib_dir.as_posix()
                )
                ...
                w_dir = w.parent
                write_quant_info(
                    w_dir / "quantize_result", ckpt["qat_model_quant_info"]
                )
        if fuse:
...
def test(
    ...
    if nndct_quant:
        ...
        if do_dump:
            assert (
                quant_mode == "test"
            ), "Quant model should be 'test' for dumping xmodel"
    ...
    # Configure
    model.eval()
    is_coco = data.endswith("coco_6cls.yaml")  # is COCO dataset
    with open(data) as f:
    ...
    if not training:
        ...
        )  # path to val/test images  #####might need change
        dataloader = create_dataloader(
            path, imgsz, batch_size, 64, opt, pad=0.5, rect=not nndct_quant
        )[0]
    ...
    if nndct_quant:
        ...
        if NndctOption.nndct_stat.value > 2:
            ...
            def do_quantize(instance, blob, name, node=None, tensor_type="input"):
                ...
                res = py_nndct.nn.NndctFixNeuron(
                    blob, blob, maxamp=[bnfp[0], bnfp[1]], method=mth
                )
                ...
                if tensor_type == "param":
                    instance.update_param_to_nndct(
                        node, name, res.cpu().detach().numpy()
                    )
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(
        tqdm(dataloader, desc=s, total=total)
    ):
        ...
        for si, pred in enumerate(output):
            ...
            if save_txt:
                ...
                x[:, :4] = scale_coords(
                    img.shape[2:], x[:, :4], shapes[si][0].shape
                ).round()  # to original
    ...
    if save_json and len(jdict):
        ...
        anno_json = glob.glob(
            "/group/dphi_algo/coco/annotations/annotations_2017/instances_val2017.json"
        )[
            0
        ]  # annotations json
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dump_onnx", action="store_true", help="dump nndct onnx xmodel"
    )
    ...
    opt.save_json |= opt.data.endswith("coco_6cls.yaml")
