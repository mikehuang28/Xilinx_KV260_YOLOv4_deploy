...
class YOLOPost:
    ...
    def forward(self, input, targets=None):
        ...
        prediction = input.reshape(
            bs, self.num_anchors, self.bbox_attrs, in_h, in_w
        ).transpose(0, 1, 3, 4, 2)
        ...
        grid_x = np.tile(
            np.tile(np.linspace(0, in_w - 1, in_w), (in_w, 1)),
            (bs * self.num_anchors, 1, 1),
        ).reshape(x.shape)
        grid_y = np.tile(
            np.tile(np.linspace(0, in_h - 1, in_h), (in_h, 1)).T,
            (bs * self.num_anchors, 1, 1),
        ).reshape(y.shape)
...
def bbox_iou(box1, box2, x1y1x2y2=True):
    ...
    # Intersection area
    inter_area = np.maximum(0, inter_rect_x2 - inter_rect_x1 + 1) * np.maximum(
        0, inter_rect_y2 - inter_rect_y1 + 1
    )
...
def non_max_suppression(prediction, num_classes, conf_thres=0.5, nms_thres=0.4):
    ...
    for image_i, image_pred in enumerate(prediction):
        ...
        print(f"max object conf = {max(image_pred[:, 4])}")
        conf_mask = image_pred[:, 4] >= conf_thres
        image_pred = image_pred[conf_mask]

        print(image_pred.shape)  # (45, 11)
        ...
        for c in unique_labels:
            ...
            if output[image_i] is None:
                ...
                output[image_i] = np.concatenate(
                    (output[image_i], max_detections), axis=0
                )

    return output


# NMS with multiple conf_thres
def non_max_suppression_with_class_thresholds(
    prediction, num_classes, class_conf_thresholds, nms_thres=0.4
):
    """
    Removes detections with lower object confidence score than 'conf_thres' and performs
    Non-Maximum Suppression to further filter detections.
    ...
    for image_i, image_pred in enumerate(prediction):
        print("--- NMS loop, different class confidence!!")
        print(f"max object conf = {max(image_pred[:, 4])}")
        ...
        for c in unique_labels:
            ...
            # Check if class has a specific confidence threshold, otherwise use a default
            if c in class_conf_thresholds:
                conf_threshold = class_conf_thresholds[c]
                print(f"c = {c}, conf = {conf_threshold}")
            # else:
            #     conf_threshold = conf_thres  # default

            # Filter detections based on class-specific confidence threshold
            detections_class = detections_class[
                detections_class[:, 4] >= conf_threshold
            ]
            print(detections_class.shape)

            # conf too small-> skip and do next class
            if detections_class.shape[0] == 0:
                continue
